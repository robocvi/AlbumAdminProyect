 ...
 */
public class ControladorInterfazAlbumes {
    ...
    private static final String BUSQUEDA_ALBUMES_FXML =
        "fxml/forma-busqueda-albumes.fxml";
    /* Vista de la forma para agregar/editar estudiantes. */
    private static final String ALBUM_FXML =
        "fxml/forma-album.fxml";
    ...
    private ControladorTablaAlbumes controladorTablaAlbumes;
    /* La base de datos. */
    private BaseDeDatosAlbumes bdd;
    /* La conexión del cliente. */
    private Conexion<Album> conexion;
    ...
    @FXML private void conectar(ActionEvent evento) {
        ...
        try {
            ...
            controladorTablaAlbumes.enfocaTabla();
        ...
        try {
            ...
            conexion = new ConexionCliente<Album>(bdd, enchufe);
    ...
    /* Agrega un nuevo album. */
    @FXML private void agregaAlbum(ActionEvent evento) {
        ControladorFormaAlbumes controlador =
            construyeDialogoAlbum("Agregar album", null);
        if (controlador == null)
            return;
        ...
        if (!controlador.isAceptado())
            return;
        bdd.agregaRegistro(controlador.getAlbum());
    }

    /* Edita un album. */
    @FXML private void editaAlbum(ActionEvent evento) {
        Album album =
            controladorTablaAlbumes.getRenglonSeleccionado();
        ControladorFormaAlbumes controlador =
            construyeDialogoAlbum("Editar album", album);
        if (controlador == null)
            return;
        ...
        if (!controlador.isAceptado())
            return;
        bdd.modificaRegistro(album, controlador.getAlbum());
    }

    /* Elimina uno o varios albumes. */
    @FXML private void eliminaAlbum(ActionEvent evento) {
        String sujeto = (seleccionados == 1) ? "album" : "albumes";
        String titulo = "Eliminar " + sujeto;
        String mensaje = (seleccionados == 1) ?
            "Esto eliminará al album seleccionado" :
            "Esto eliminará a los albumes seleccionados";
        if (!dialogoDeConfirmacion(titulo, mensaje, "¿Está seguro?",
                                   "Eliminar " + sujeto,
                                   "Conservar " + sujeto))
            return;
        for (Album album : controladorTablaAlbumes.getSeleccion())
            bdd.eliminaRegistro(album);
    }

    /* Busca albumes. */
    @FXML private void buscaAlbumes(ActionEvent evento) {
        try {
            ClassLoader cl = getClass().getClassLoader();
            FXMLLoader cargador =
                new FXMLLoader(cl.getResource(BUSQUEDA_ALBUMES_FXML));
            AnchorPane pagina = (AnchorPane)cargador.load();

            Stage escenario = new Stage();
            escenario.setTitle("Buscar albumes");
            escenario.initOwner(this.escenario);
            escenario.initModality(Modality.WINDOW_MODAL);
            Scene escena = new Scene(pagina);
            escenario.setScene(escena);

            ControladorFormaBusquedaAlbumes controlador;
            ...
            Lista<Album> resultados =
                bdd.buscaRegistros(controlador.getCampo(),
                                   controlador.getValor());

            controladorTablaAlbumes.seleccionaRenglones(resultados);
        } catch (IOException | IllegalStateException e) {
            String mensaje =
                String.format("Ocurrió un error al tratar de " +
                              "cargar el diálogo '%s'.",
                              BUSQUEDA_ALBUMES_FXML);
            dialogoError("Error al cargar interfaz", mensaje);
        }
    }
    ...
    @FXML private void acercaDe(ActionEvent evento) {
        ...
        dialogo.setTitle("Acerca de Administrador de Albumes.");
        dialogo.setHeaderText(null);
        dialogo.setContentText("Aplicación para administrar albumes.\n"  +
     ...
     * @param controladorTablaAlbumes el controlador de tabla.
     */
    public void setControladorTablaAlbumes(ControladorTablaAlbumes
                                               controladorTablaAlbumes) {
        this.controladorTablaAlbumes = controladorTablaAlbumes;
        controladorTablaAlbumes.agregaEscuchaSeleccion(
    ...
    private void manejaEventoBaseDeDatos(EventoBaseDeDatos evento,
        ...
        case REGISTRO_AGREGADO:
            Platform.runLater(() -> agregaAlbum(album1));
            break;
        case REGISTRO_ELIMINADO:
            Platform.runLater(() -> eliminaAlbum(album1));
            break;
    ...
    private void manejaEventoConexion(Conexion<Album> conexion,
    ...
    /* Construye un diálogo para crear o editar un album. */
    private ControladorFormaAlbumes
    construyeDialogoAlbum(String titulo,
                               Album album) {
        try {
            ClassLoader cl = getClass().getClassLoader();
            FXMLLoader cargador =
                new FXMLLoader(cl.getResource(ALBUM_FXML));
            AnchorPane pagina = (AnchorPane)cargador.load();
            ...
            ControladorFormaAlbumes controlador = cargador.getController();
            controlador.setEscenario(escenario);
            controlador.setAlbum(album);
    ...
    /* Agrega un album a la tabla. */
    private void agregaAlbum(Album album) {
        controladorTablaAlbumes.agregaRenglon(album);
    }

    /* Elimina un album de la tabla. */
    private void eliminaAlbum(Album album) {
        controladorTablaAlbumes.eliminaRenglon(album);
    }

    /* Reordena la tabla. */
    private void reordenaTabla() {
        controladorTablaAlbumes.reordena();
    }

    /* Limpia la tabla. */
    private void limpiaTabla() {
        controladorTablaAlbumes.limpiaTabla();
    }
}
